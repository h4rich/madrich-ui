...
function QnA() {
  return (
    <section className="w-full py-32 bg-card-alt font-grotesk relative overflow-hidden">
      <div className="max-w-4xl mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-5xl md:text-6xl font-black uppercase text-foreground mb-6">
            Got{" "}
            <span className="text-background bg-foreground px-2 -rotate-1 inline-block">
              Questions?
            </span>
          </h2>
        ...
        <Accordion type="single" collapsible className="space-y-4">
          {faqs.map((faq, index) => (
            <div key={index} className="group">
              <AccordionItem
                value={`item-${index}`}
                className="bg-card-surface border-3 border-foreground shadow-neo-md data-[state=open]:shadow-[2px_2px_0px_0px_rgb(var(--neo-shadow-color))] data-[state=open]:translate-x-[4px] data-[state=open]:translate-y-[4px] transition-all duration-200"
              >
                ...
                <AccordionContent className="text-lg font-medium px-6 py-5 bg-card-surface">
                  {faq.answer}
                </AccordionContent>
              </AccordionItem>
            </div>
      ...
      <div className="absolute top-20 left-[-20px] w-32 h-32 bg-[#80C551] rounded-full border-3 border-foreground shadow-neo-4 z-0 hidden md:block"></div>
      <div className="absolute bottom-20 right-[-20px] w-40 h-40 bg-[#E9DCFE] rotate-12 border-3 border-foreground shadow-neo-4 z-0 hidden md:block"></div>
      <div className="absolute top-1/2 right-10 w-16 h-16 bg-[#FF7051] rotate-45 border-3 border-foreground shadow-neo-4 z-0 hidden md:block"></div>
    </section>
  );
}
